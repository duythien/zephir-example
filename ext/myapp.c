...
static PHP_MINIT_FUNCTION(myapp)
{
#if PHP_VERSION_ID < 50500
 ...
 */
static void php_zephir_init_globals(zend_myapp_globals *zephir_globals TSRMLS_DC)
{
...
static PHP_RINIT_FUNCTION(myapp)
{
	...
	zend_myapp_globals *zephir_globals_ptr = ZEPHIR_VGLOBAL;
...
static PHP_MINFO_FUNCTION(myapp)
{
	...
	php_printf("%s", PHP_MYAPP_DESCRIPTION);
	php_info_print_box_end();

	php_info_print_table_start();
	php_info_print_table_header(2, PHP_MYAPP_NAME, "enabled");
	php_info_print_table_row(2, "Author", PHP_MYAPP_AUTHOR);
	php_info_print_table_row(2, "Version", PHP_MYAPP_VERSION);
	php_info_print_table_row(2, "Powered by Zephir", "Version " PHP_MYAPP_ZEPVERSION);
