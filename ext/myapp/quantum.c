...
PHP_METHOD(Myapp_Quantum, Harmos) {
	...
	while (1) {
		...
		zephir_array_update_long(&psr, i, &_11, PH_COPY | PH_SEPARATE, "myapp/quantum.zep", 37);
		ZEPHIR_INIT_NVAR(_12);
		array_init_size(_12, 2);
		ZEPHIR_INIT_NVAR(_0);
		ZVAL_DOUBLE(_0, item_psi);
		zephir_array_fast_append(_12, _0);
		zephir_array_update_long(&psi, i, &_12, PH_COPY | PH_SEPARATE, "myapp/quantum.zep", 38);
		ZEPHIR_INIT_NVAR(_13);
		array_init_size(_13, 2);
		ZEPHIR_INIT_NVAR(_0);
		ZVAL_DOUBLE(_0, (double) ((double) (5.0 * x) * x));
		zephir_array_fast_append(_13, _0);
		zephir_array_update_long(&v, i, &_13, PH_COPY | PH_SEPARATE, "myapp/quantum.zep", 39);
		x = (x + dx);
		i++;
	}
	i = 1;
		...
		while (1) {
			...
			if (_18 <= _19) {
				while (1) {
					...
					zephir_array_update_long(&p2, i, &_8, PH_COPY | PH_SEPARATE, "myapp/quantum.zep", 51);
				}
			}
			_35 = (751 - 1);
			_34 = 1;
